...
function Repair-TranslationText($value) {
    $text = [string]$value
    if ([string]::IsNullOrWhiteSpace($text) -or $text -match '[\u3400-\u9fff]') { return $text }
    try {
        $latin1 = [System.Text.Encoding]::GetEncoding(28591)
        $bytes = $latin1.GetBytes($text)
        if ($latin1.GetString($bytes) -ne $text) { return $text }
        $fixed = [System.Text.UTF8Encoding]::new($false, $true).GetString($bytes)
        if ($fixed -match '[\u3400-\u9fff]') { return $fixed }
    } catch { }
    return $text
}
...
if ($null -ne $loadedCache) {
    $cacheRepaired = $false
    foreach ($property in $loadedCache.PSObject.Properties) {
        $original = [string]$property.Value
        $repaired = Repair-TranslationText $original
        $script:cache[$property.Name] = $repaired
        if ($repaired -cne $original) { $cacheRepaired = $true }
    }
    if ($cacheRepaired) {
        try {
            $backupPath = Join-Path $script:storage 'translations.before-v1.5.json'
            if (-not (Test-Path -LiteralPath $backupPath)) { [System.IO.File]::Copy($script:cachePath, $backupPath, $false) }
...
function Start-TranslationJobs {
    ...
    while ($script:translationJobs.Count -lt 2 -and $script:translationQueue.Count -gt 0) {
        ...
        $job = Start-Job -ArgumentList $translationArguments -ScriptBlock {
            ...
            try {
                $response = Invoke-WebRequest -Uri $endpoint -Method Post -Body ([System.Text.Encoding]::UTF8.GetBytes($body)) -ContentType 'application/json; charset=utf-8' -Headers $headers -TimeoutSec 30 -UseBasicParsing
                $reply = [System.Text.Encoding]::UTF8.GetString($response.RawContentStream.ToArray()) | ConvertFrom-Json
...
'@
    ...
    $dialog.FindName('TestLlm').Add_Click({
        ...
        try {
            $response = Invoke-WebRequest -Uri $address -Method Post -Body ([System.Text.Encoding]::UTF8.GetBytes($payload)) -ContentType 'application/json; charset=utf-8' -Headers $headers -TimeoutSec 20 -UseBasicParsing
            $reply = [System.Text.Encoding]::UTF8.GetString($response.RawContentStream.ToArray()) | ConvertFrom-Json
            $translated = [string]$reply.choices[0].message.content
            if ([string]::IsNullOrWhiteSpace($translated)) { throw '响应中没有译题' }
