...
function Start-TranslationJobs {
    ...
    while ($script:translationJobs.Count -lt 2 -and $script:translationQueue.Count -gt 0) {
        ...
        $job = Start-Job -ArgumentList $paper.Title, [string]$script:llm.Endpoint, [string]$script:llm.Model, $key -ScriptBlock {
            ...
            [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12
            $body = @{ model = $model; temperature = 0; max_tokens = 120; messages = @(@{ role = 'system'; content = '将英文教育技术学术论文标题准确翻译成简体中文。只返回中文译题，不加引号、注释或原文。' }, @{ role = 'user'; content = $title }) } | ConvertTo-Json -Depth 6
            $headers = @{}
...
'@
    ...
    $dialog.FindName('TestLlm').Add_Click({
        ...
        $payload = @{ model = $modelName; temperature = 0; max_tokens = 100; messages = @(@{ role = 'system'; content = '将英文标题翻译为简体中文，只返回译题。' }, @{ role = 'user'; content = 'Learning with technology' }) } | ConvertTo-Json -Depth 5
        $headers = @{}
