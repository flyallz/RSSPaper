...
function Start-Refresh {
    ...
    $script:refreshJob = Start-Job -ArgumentList $feedJson, [string]$script:network.Proxy -ScriptBlock {
        param($json, $proxyAddress)
        Add-Type -AssemblyName System.Net.Http
        [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12
        $feeds = @(ConvertFrom-Json $json)
...
function Start-TranslationJobs {
    ...
    while ($script:translationJobs.Count -lt 2 -and $script:translationQueue.Count -gt 0) {
        ...
        $key = Read-ApiKey
        $job = Start-Job -ArgumentList $paper.Title, [string]$script:llm.Endpoint, [string]$script:llm.Model, $key -ScriptBlock {
            param($title, $endpoint, $model, $apiKey)
            [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.SecurityProtocolType]::Tls12
            $requestBody = @{ model = $model; temperature = 0; max_tokens = 256; messages = @(@{ role = 'system'; content = '将英文教育技术学术论文标题准确翻译成简体中文。只返回中文译题，不加引号、注释或原文。' }, @{ role = 'user'; content = $title }) }
            if (([uri]$endpoint).Host -eq 'api.deepseek.com') { $requestBody['thinking'] = @{ type = 'disabled' } }
...
'@
    ...
    $dialog.FindName('TestLlm').Add_Click({
        ...
        $requestBody = @{ model = $modelName; temperature = 0; max_tokens = 256; messages = @(@{ role = 'system'; content = '将英文标题翻译为简体中文，只返回译题。' }, @{ role = 'user'; content = 'Learning with technology' }) }
        if (([uri]$address).Host -eq 'api.deepseek.com') { $requestBody['thinking'] = @{ type = 'disabled' } }
        $payload = $requestBody | ConvertTo-Json -Depth 5
        $headers = @{}
        ...
        try {
            ...
            $llmMessage.Text = '测试失败：' + $(if ($status -gt 0) { 'HTTP ' + $status + '：' } else { '' }) + $hint
        }
    })
