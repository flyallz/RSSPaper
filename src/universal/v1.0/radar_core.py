"""Storage and scholarly-source adapters for the Windows Journal Radar app."""
...
APP_NAME = "期刊雷达"
USER_AGENT = "JournalRadar/1.0 (Windows; personal research reader)"
...
def _paper(source: dict, title: str, link: str, date: str) -> dict:
    return {
    ...
    for item in root.iter():
        ...
        for child in item:
            ...
            if field == "title" and not title:
                ...
                date = _date(_text(child))
        if title and valid_http_url(link):
            papers.append(_paper(source, title, link, date))
        if len(papers) >= 60:
            break
    return papers
    ...
    for item in items:
        ...
        for field in ("published-online", "published", "published-print", "issued"):
            ...
            if parts and parts[0]:
                values = parts[0]
                try:
                    date = datetime(int(values[0]), int(values[1]) if len(values) > 1 else 1, int(values[2]) if len(values) > 2 else 1).date().isoformat()
